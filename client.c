...
//char salt[20]="bvOMl3ypNROY3e94";
//char full_salt[30];

const char *salt="O.lowH3gsJ/PYwrq";
const char *target="$6$O.lowH3gsJ/PYwrq$6rXPxKAiKDGVFdCl4bngsjr3ORJa1n98kLGi0XC8BpO.A6j0GDEaaORijqRE.yGp3SUsbEZ1xfaXg2GyCT2q61";


//const char *target = "$6$bvOMl3ypNROY3e94$a.MmzoYrLBHJqgZTsITxjFhIeD/AQ3W53eB.3e./QrnLbNQVJx8X1.ojiTXsgXBo.mexhcs6Sz0AgQ7LMjIm.."; // Искомая комбинация
...
time_t s_,e_;
// Функция для поиска комбинации
void search_combinations(const char *start, const char *target) {
    char current[MAX_LENGTH + 1];
    strncpy(current, start, MAX_LENGTH);
    current[MAX_LENGTH] = '\0';
 s_=time(NULL);
    for (int i = 0; i < CHUNK_SIZE; i++) {
        //char *encrypted = crypt(current, full_salt);
//        s_=time(NULL);
        char *cp = sha512_crypt (current, salt);
//        e_=time(NULL);
//        printf("%lu ",s_-e_);
        if (strcmp (cp, target) == 0)
        {
            printf("Найдена комбинация: %s\n", current);
            f=1;
            if (send(client_socket, current, strlen(current)+1, 0) < 0) {
...
//    const char *target = "41"; // Искомая комбинация
    char buffer[MAX_LENGTH + 1];

    while (1) {
        // Получение текущей комбинации от сервера
...
//        printf("%s\n",buffer);
        //sleep(3);
        search_combinations(buffer, target);
        if ( f == 1){
    	    if (send(client_socket, "FOUND", 6, 0) < 0) {
        	perror("Ошибка отправки данных");
        	break;
    	    }
    	    close(client_socket);
